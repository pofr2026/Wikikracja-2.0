...
class NewPersonEmailTest(TestCase):
    ...
    def test_send_email_to_all_sends_exactly_one_email_on_repeated_calls(self):
        ...
        self.assertEqual(len(mail.outbox), 2, (f"Expected 2 emails (one per event), got {len(mail.outbox)}."))
    ...
    def test_no_email_when_no_active_users(self):
        ...
        self.assertLessEqual(len(mail.outbox), 1, (f"Expected at most 1 email with no active users, got {len(mail.outbox)}."))
...
class ChatMessagesEmailTest(TestCase):
    ...
    def test_one_email_per_user_with_new_messages(self):
        ...
        self.assertEqual(len(recipient_emails), 1, (f"Expected 1 email for recipient, got {len(recipient_emails)}. "
    ...
    def test_no_email_when_no_new_messages(self):
        ...
        recipient_emails = [e for e in mail.outbox if self.recipient.email in e.bcc]
        self.assertEqual(len(recipient_emails), 0, (f"Expected 0 emails when no new messages, got {len(recipient_emails)}."))
    ...
    def test_one_email_aggregates_multiple_messages(self):
        ...
        recipient_emails = [e for e in mail.outbox if self.recipient.email in e.bcc]
        self.assertEqual(len(recipient_emails), 1, (f"Expected 1 aggregated email, got {len(recipient_emails)}."))
    ...
    def test_running_command_twice_sends_two_emails(self):
        ...
        recipient_emails = [e for e in mail.outbox if self.recipient.email in e.bcc]
        self.assertEqual(len(recipient_emails), 2, (f"Expected 2 emails (one per run), got {len(recipient_emails)}."))
    ...
    def test_sender_does_not_receive_own_message_email(self):
        ...
        self.assertEqual(len(sender_emails), 0, (f"Sender should not receive email for own message, got {len(sender_emails)}."))
